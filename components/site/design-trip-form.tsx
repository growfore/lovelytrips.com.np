...
const itineraryFormSchema = z
  .object({
    ...
    duration: z.enum(DURATION_VALUES as [string, ...string[]], {
...
const STEPS = [
  { label: "Trip Basics", icon: Compass },
  ...
  const onSubmit = async (data: ItineraryFormValues) => {
    ...
    try {
      const locationsSummary = data.letUsChooseLocations
        ...
        : data.locations
            .filter((l) => l.name)
            .map((l) => `${l.name} (${l.days} day${Number(l.days) > 1 ? "s" : ""})`)
            .join(", ")
      ...
      const text = [
        ...
        `No. of Travellers:     ${data.numberOfTravellers || "N/A"}`,
        ``,
        `── Preferences ──`,
        `Inclusions:            ${data.inclusions.join(", ") || "None selected"}`,
      ...
      const html = `
        <h2>Custom Itinerary Request from ${escapeHtml(data.fullName)}</h2>
        <p><strong>Email:</strong> ${escapeHtml(data.email)}</p>
        <p><strong>Phone:</strong> ${escapeHtml(data.phone || "Not provided")}</p>
        <p><strong>Duration:</strong> ${escapeHtml(data.duration)}</p>
        <p><strong>Experience:</strong> ${escapeHtml(data.experienceType.join(", "))}</p>
        <p><strong>Start Date:</strong> ${escapeHtml(data.startDate)}</p>
        <p><strong>Locations:</strong> ${escapeHtml(locationsSummary)}</p>
        <p><strong>Group Type:</strong> ${escapeHtml(data.groupType)}</p>
        <p><strong>Travellers:</strong> ${escapeHtml(String(data.numberOfTravellers || "N/A"))}</p>
        <p><strong>Inclusions:</strong> ${escapeHtml(data.inclusions.join(", ") || "None")}</p>
        <p><strong>Accommodation:</strong> ${escapeHtml(data.accommodationPreferences.join(", "))}</p>
        <p><strong>Food:</strong> ${escapeHtml(data.foodPreferences.join(", "))}</p>
        <hr/>
      ...
      toast.success("Request sent! Our team will craft your custom itinerary within 24 hours.")
        ...
        <form onSubmit={form.handleSubmit(onSubmit)}>
          {step === 0 && (
            <div className="space-y-5">
              <div>
                <h2 className="text-2xl font-bold text-ink mb-1">Trip Basics</h2>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-ink font-semibold text-sm">Total Duration *</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-ink font-semibold text-sm">Experience / Trek Type *</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-ink font-semibold text-sm">Preferred Start Date *</FormLabel>
                    <FormControl>
          ...
          {step === 1 && (
            <div className="space-y-5">
              <div>
                <h2 className="text-2xl font-bold text-ink mb-1">Locations</h2>
                <p className="text-sm text-muted-foreground mb-6">
              ...
              {!letUsChoose && (
                <div className="space-y-3">
                  <p className="text-sm font-semibold text-ink">Your Locations</p>
          ...
          {step === 2 && (
            <div className="space-y-5">
              <div>
                <h2 className="text-2xl font-bold text-ink mb-1">Your Group</h2>
                <p className="text-sm text-muted-foreground mb-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-ink font-semibold text-sm">Group Type *</FormLabel>
              ...
              <div className={groupType === "solo" || groupType === "couple" ? "hidden" : ""}>
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-ink font-semibold text-sm">Number of Travellers *</FormLabel>
                      <Select onValueChange={field.onChange} value={field.value}>
                        <FormControl>
                          <SelectTrigger className="w-full">
          ...
          {step === 3 && (
            <div className="space-y-8">
              <div>
                <h2 className="text-2xl font-bold text-ink mb-1">Preferences</h2>
                <p className="text-sm text-muted-foreground mb-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-ink font-semibold text-sm mb-3 block">Pick Your Inclusions</FormLabel>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-ink font-semibold text-sm">Other Mentions / Special Requests</FormLabel>
                    <FormControl>
                      <Textarea
          ...
          {step === 4 && (
            <div className="space-y-5">
              <div>
                <h2 className="text-2xl font-bold text-ink mb-1">Contact Details</h2>
                <p className="text-sm text-muted-foreground mb-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-ink font-semibold text-sm">Full Name *</FormLabel>
                      <FormControl>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className="text-ink font-semibold text-sm">Email *</FormLabel>
                      <FormControl>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel className="text-ink font-semibold text-sm">Phone (optional)</FormLabel>
                    <FormControl>
              ...
              <div className="rounded-lg border border-border bg-muted/50 p-4 space-y-2 text-sm">
                <p className="font-semibold text-ink mb-3">Your Itinerary Summary</p>
                {(
                  [
                  ...
                  .map(([label, value]) => (
                    <div key={label} className="flex gap-2 text-muted-foreground">
                      <span className="font-medium text-ink w-28 shrink-0">{label}:</span>
          ...
          <div className="flex items-center justify-between mt-8 pt-6 border-t border-border">
            ...
            {step < STEPS.length - 1 ? (
              <Button type="button" onClick={goNext} className="bg-primary hover:bg-primary/90 text-primary-foreground">
                Continue
              </Button>
            ) : (
