        ...
        <div className="space-y-6">
          {activity.additionalInfo.map((info, i) => (
            <div key={i} className="bg-white/80 rounded-xl p-6 md:p-8 border border-ink/5">
              <h3 className="text-xl font-script text-ink mb-4">{info.title}</h3>
              <div
      ...
      <div className="relative z-10 max-w-3xl mx-auto">
        ...
        <Accordion type="single" collapsible className="space-y-3">
          {allFaqs.map((faq, i) => (
            ...
            >
              <AccordionTrigger className="text-left text-sm md:text-base font-medium text-ink hover:no-underline cursor-pointer">
                {faq.question}
              </AccordionTrigger>
